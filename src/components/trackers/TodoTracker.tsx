...
// useIndexedDB: Speichert Todo-Gruppen in IndexedDB unter einem einzigen Key "data"
function useIndexedDB<T>(storeName: string, initialValue: T): [T, (value: T | ((prev: T) => T)) => void] {
  ...
  useEffect(() => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      // Store ohne keyPath – wir speichern alles später unter dem Key "data".
    ...
    request.onsuccess = (event) => {
      ...
      getRequest.onsuccess = () => {
        if (getRequest.result) {
          console.log('Existing data found in store:', getRequest.result);
          setData(getRequest.result);
        } else {
          console.log('No existing data; writing initialValue...');
    ...
    // Cleanup: DB schließen, wenn Komponente ungemountet wird
    return () => {
      if (db) {
  ...
  const setValue = (valueOrFunction: T | ((prev: T) => T)) => {
    ...
    request.onsuccess = (event) => {
      ...
      // Wir speichern das komplette Array "newValue" unter dem Key "data"
...
const TodoTracker: React.FC = () => {
  const [todoGroups, setTodoGroups] = useIndexedDB<TodoGroup[]>('todoGroups', [
    {
      id: Date.now(),
      ...
      setTodoGroups(prevGroups =>
        prevGroups.map(group =>
          group.id === groupId
            ? {
                ...group,
                todos: [newTodo, ...group.todos].sort(
  ...
  // Falls neue Gruppen existieren, die noch nicht im selectedPriorities-Objekt sind,
  // wird ihnen standardmäßig "none" zugewiesen
  React.useEffect(() => {
        ...
        <div className="bg-gray-700 rounded-lg p-3 mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-300">
              You have {archivedTasksCount} archived task
              {archivedTasksCount !== 1 ? 's' : ''}
            </span>
          </div>
          <button
      ...
      <div className="grid gap-4 sm:gap-6">
        {todoGroups.map(group => (
          <div key={group.id} className="bg-gray-800 rounded-lg p-3 sm:p-4 relative">
            ...
            {/* Nur im aktiven Modus (viewArchived: false) Neues Task-Feld */}
            {!filterOptions.viewArchived && (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <div className="flex border border-gray-600 rounded overflow-hidden">
                          <button
                            ...
                            className={`p-1 ${
                              todo.priority === 'high' ? 'bg-red-500/30' : 'hover:bg-gray-600'
                            }`}
                            title="High Priority"
                          >
                            <ArrowUp
                              className={`w-3 h-3 ${
                                todo.priority === 'high' ? 'text-red-400' : 'text-red-500'
                              }`}
                            />
                          </button>
                          <button
                          ...
                          >
                            <Circle
                              className={`w-3 h-3 ${
                                todo.priority === 'medium' ? 'text-yellow-400' : 'text-yellow-500'
                              }`}
                            />
                          </button>
                          <button
                            ...
                            className={`p-1 ${
                              todo.priority === 'low' ? 'bg-green-500/30' : 'hover:bg-gray-600'
                            }`}
                            title="Low Priority"
                          >
                            <ArrowDown
                              className={`w-3 h-3 ${
                                todo.priority === 'low' ? 'text-green-400' : 'text-green-500'
                              }`}
                            />
                          </button>
                          <button
                    ...
                    <div className="space-y-2 ml-4 sm:ml-6">
                      {todo.notes.map(note => (
                        <div
                          key={note.id}
                          className="text-sm text-gray-400 flex justify-between items-start gap-2"
                        >
